...
function faxSheetHtml(rows: PrintRow[], pickupDate: string, options: FaxPrintOptions = {}) {
  ...
  <section class="print-page fax-page-print">
    ...
    ${sorted.map((row, index) => `<tr><td>${index + 1}</td><td>${formatDate(row.caseRow.payment_date)}</td><td>${escapeHtml(row.caseRow.receipt_no)}</td><td>${row.appItem?.requires_ic_card ? 'V' : ''}</td><td>1</td><td></td><td>${escapeHtml(row.caseRow.foreign_no_last5)}</td><td>${row.appItem?.requires_old_card ? 'V' : ''}</td><td>${escapeHtml(row.caseRow.employer_name)}</td><td>${escapeHtml(row.caseRow.worker_name)}</td><td>${escapeHtml(row.caseRow.handler_name)}</td><td class="small">${row.caseRow.receipt_order ?? ''}</td></tr>`).join('')}
    </tbody></table>
