...
- 居留證繳費待繳案件金額可直接修改，支援千分位與前後空白清除。
- 金額錯誤時不可扣款，會顯示「金額格式不正確，請重新輸入。」
- 管理員可將待繳案件從繳費頁移除，案件主資料仍保留在案件查詢，狀態顯示「已從繳費頁移除」。
- 扣款帳號會依仲介自動帶入：單一帳戶直接帶入，多帳戶優先帶入預設扣款帳戶，沒有預設時需人工選擇。
- 帳戶設定新增「預設扣款帳戶」。
- 金額修改、待繳移除、扣款與帳戶餘額異動皆保留操作紀錄。
- 本版需先執行 `supabase/migrations/202607020005_arc_v13_v30_payment_amount_delete.sql`。
...
- 待繳案件依仲介分成獨立區塊。
- 每個仲介區塊只顯示該仲介案件。
- 每個仲介區塊顯示：扣款帳號、帳號後五碼、目前餘額、待繳件數、待繳總金額、已選金額。
- 同一仲介多個帳戶會在該仲介區塊內分開顯示，不合併餘額。
- 扣款帳號依仲介自動帶入，若有多帳戶優先帶入預設帳戶；若無預設，需由該區塊下拉選擇。
- 修改某一仲介區塊的扣款帳號，不會影響其他仲介區塊。
- 每個仲介區塊可獨立一鍵勾選、一鍵取消與建立繳費扣款。
- 搜尋後仍維持仲介分區顯示，不重建搜尋 input。
- 金額修改、刪除、扣款仍沿用 V30 權限與異動紀錄邏輯。

本版不需要新增 Supabase SQL；若已跑過 V30 SQL，可直接覆蓋前端。
